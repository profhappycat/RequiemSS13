/datum/adaptation/unnatural/aclaws
	name = "Acidic Claws"
	var/obj/item/melee/vampirearms/knife/protean/left_weapon
	var/obj/item/melee/vampirearms/knife/protean/right_weapon

/datum/adaptation/unnatural/aclaws/activate(mob/living/carbon/owner)
	. = ..()
	playsound(get_turf(owner), 'sound/effects/dismember.ogg', 100, TRUE, -6)
	owner.visible_message(span_alert("[owner]'s hands begin to crack and distort, growing twisted claws that ooze something unnatural!"))
...
/datum/adaptation/unnatural/aclaws/deactivate(mob/living/carbon/owner)
	. = ..()
	playsound(get_turf(owner), 'sound/effects/dismember.ogg', 100, TRUE, -6)
