/obj/item/supplypod_beacon_vtr
	...
	mouse_opacity = 0
	var/obj/machinery/computer/cargo/express_vtr/express_console
	var/linked = FALSE
	var/ready = FALSE
	var/launched = FALSE

/obj/item/supplypod_beacon_vtr/proc/update_status(consoleStatus)
	switch(consoleStatus)
		...
		if (SP_LAUNCH)
			...
			addtimer(CALLBACK(src, PROC_REF(endLaunch)), 5 MINUTES)//wait 3.3 seconds (time it takes for supplypod to land), then update icon
		if (SP_UNLINK)
			linked = FALSE
...
/obj/item/supplypod_beacon_vtr/update_overlays()
	. = ..()
	if (launched)
		. += "sp_green"
	else if (ready)
...
/obj/item/supplypod_beacon_vtr/proc/endLaunch()
	launched = FALSE
	update_status()

/obj/item/supplypod_beacon_vtr/examine(user)
	. = ..()
	if(!express_console)
...
/obj/item/supplypod_beacon_vtr/Destroy()
	if(express_console)
		express_console.beacon = null
	return ..()

/obj/item/supplypod_beacon_vtr/proc/unlink_console()
	if(express_console)
		express_console.beacon = null
...
/obj/item/supplypod_beacon_vtr/proc/link_console(obj/machinery/computer/cargo/express_vtr/C, mob/living/user)
...
/obj/item/supplypod_beacon_vtr/proc/altlink_console(obj/machinery/computer/cargo/express_vtr/C)
	if (C.beacon)//if new console has a beacon, then...
		C.beacon.unlink_console()//unlink the old beacon from new console
	if (express_console)//if this beacon has an express console
		express_console.beacon = null//remove the connection the expressconsole has from beacons
...
/obj/item/supplypod_beacon_vtr/AltClick(mob/user)
	return

/obj/item/supplypod_beacon_vtr/attackby(obj/item/W, mob/user)
